...
class Scheduler:
    # Task 1: variable to track iteration count for CFS scheduling
    num_iters = 0
    ...
    def _get_priority(self,
        ...
        """
        # Task 2.1: Change priority from arrival_time to num_tokens_generated for fairness
        # Lower token count gets scheduled first
        num_tokens_generated = sum(
            seq.get_output_len() for seq in seq_group.get_seqs()
        )
        return seq_group.priority, num_tokens_generated
    ...
    def _schedule_priority_preemption(
        ...
        if waiting_queue:
            ...
            while running_queue and self._get_priority(
                ...
                # Task 2.4: Swap victim to CPU preserving KV cache instead of recomputing
                # Preempt and send to swapped queue to preserve KV cache
                preemption_mode = self._preempt(vseq_group, blocks_to_swap_out)
                if preemption_mode == PreemptionMode.SWAP:
                    self.swapped.append(vseq_group)
                else: # if recompute, send to waiting queue
                    waiting_queue.appendleft(vseq_group)
                force_preemption_count += 1
            # Put the sequence back into the waiting queue
            waiting_queue.appendleft(seq_group)
        else:
            # Task 2.4: ELSE block - check SWAPPED queue for priority checks
            # When no waiting requests, bring back swapped requests if they have higher priority
            swapped_queue = self.swapped
            
            if swapped_queue and running_queue:
                # Sort swapped queue by priority in ascending order
                sorted_swapped = sorted(swapped_queue, key=self._get_priority)
                
                # Check if highest priority swapped request has higher priority than lowest priority running request
                highest_priority_swapped = sorted_swapped[0]
                lowest_priority_running = running_queue[-1]
                
                if self._get_priority(highest_priority_swapped) < self._get_priority(lowest_priority_running):
                    # If swapped request has HIGHER priority than running request
                    # Preempt the lowest priority running request to make room for swapped request
                    
                    # Remove victim from running queue
                    victim_seq_group = running_queue.pop()
                    
                    # Adjust budget
                    num_running_tokens_uncached, _ = (
                        self._get_num_new_uncached_and_cached_tokens(
                            victim_seq_group, SequenceStatus.RUNNING, False, budget))
                    budget.subtract_num_batched_tokens(
                        victim_seq_group.request_id, num_running_tokens_uncached)
                    num_running_seqs = victim_seq_group.get_max_num_running_seqs()
                    budget.subtract_num_seqs(victim_seq_group.request_id,
                                             num_running_seqs)
                    
                    # Preempt the victim
                    preemption_mode = self._preempt(victim_seq_group, blocks_to_swap_out)
                    if preemption_mode == PreemptionMode.SWAP:
                        self.swapped.append(victim_seq_group)
                    else:
                        waiting_queue.appendleft(victim_seq_group)
                    force_preemption_count += 1
        ...
        logger.info("Pending queue size: (%s)", (len(self.waiting) + len(self.swapped)))
    ...
    def _schedule_cfs(self) -> SchedulerOutputs:
        """Schedule queued requests with CFS (Completely Fair Scheduler).

        Task 2.2 & 2.3: CFS scheduler for fair TTFT distribution.
        Proactively preempts long-running requests to give new/swapped requests fair access.
        """
        ...
        # Task 2.3: Always call priority preemption before scheduling prefills and remove the if statements
    ...
    def _schedule(self) -> SchedulerOutputs:
        """Schedule queued requests."""
        # Task 1 : Every 10th iteration, schedule using CFS scheduler
