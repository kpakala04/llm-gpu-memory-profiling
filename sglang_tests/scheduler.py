...
class Scheduler(
    ...
    def __init__(
        ...
        self.cfs_num_iters = 0  # CFS iteration counter for preemption frequency
        ...
        # CFS SWAP mode: queue for requests with KV cache on CPU
        self.swapped_queue: List[Req] = []
        self.swapped_host_indices: Dict[str, torch.Tensor] = {}  # rid -> host indices
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        # CFS proactive preemption: preempt running requests with more tokens
        # Only check every 10 iterations like vLLM to avoid thrashing
        if self.schedule_policy == "cfs" and not self.running_batch.is_empty():
            self.cfs_num_iters += 1
            if self.cfs_num_iters % 10 == 0:
                preemption_count = self.cfs_proactive_preemption()
                if preemption_count > 0:
                    logger.info(
                        f"CFS: Preempted {preemption_count} request(s) for fair scheduling"
                    )

        # CFS SWAP mode: Try to swap in requests from swapped queue
        if self.schedule_policy == "cfs" and self.swapped_queue:
            swapped_in = []
            for req in self.swapped_queue:
                if self._cfs_swap_in(req):
                    swapped_in.append(req)
                    logger.info(f"CFS SWAP: Restored KV cache from CPU for req {req.rid[:8]}")
                    break  # Only swap in one at a time
            for req in swapped_in:
                self.swapped_queue.remove(req)
                self.waiting_queue.insert(0, req)  # Add to front of waiting queue
    ...
    def cfs_proactive_preemption(self) -> int:
        """
        CFS proactive preemption with HiCache SWAP mode:
        - If HiCache enabled: backup KV cache to CPU (SWAP mode)
        - Otherwise: fallback to RECOMPUTE mode
        
        Returns the number of preempted requests.
        """
        if not self.waiting_queue or self.running_batch.is_empty():
            return 0
        
        # Find waiting request with fewest tokens (highest CFS priority)
        waiting_sorted = sorted(self.waiting_queue, key=lambda r: len(r.output_ids))
        if not waiting_sorted:
            return 0
        
        highest_priority_waiting = waiting_sorted[0]
        waiting_tokens = len(highest_priority_waiting.output_ids)
        
        # Sort running requests by tokens (descending) - preempt longest first
        sorted_indices = list(range(len(self.running_batch.reqs)))
        sorted_indices.sort(
            key=lambda i: len(self.running_batch.reqs[i].output_ids),
            reverse=True
        )
        
        # Find requests to preempt (just 1 for stability)
        swapped_reqs = []
        for idx in sorted_indices:
            req = self.running_batch.reqs[idx]
            running_tokens = len(req.output_ids)
            
            # Skip requests that have already been freed
            if req.kv_committed_freed:
                continue
            
            # Only preempt if this running request has more tokens than waiting
            if running_tokens > waiting_tokens:
                # Try HiCache SWAP mode first
                if self.enable_hierarchical_cache and self._cfs_swap_out(req):
                    swapped_reqs.append(req)
                    sorted_indices.remove(idx)
                    logger.info(f"CFS SWAP: Backed up {running_tokens} tokens to CPU for req {req.rid[:8]}")
                else:
                    # Fallback to RECOMPUTE mode
                    self.running_batch.release_req(
                        idx, len(sorted_indices) - len(swapped_reqs) - 1, self.server_args
                    )
                    swapped_reqs.append(req)
                    sorted_indices.remove(idx)
                break  # Only preempt one at a time
        
        if not swapped_reqs:
            return 0
        
        # Filter batch to remove preempted requests
        self.running_batch.filter_batch(keep_indices=sorted_indices)
        
        # Add to appropriate queue
        for req in swapped_reqs:
            if req.rid in self.swapped_host_indices:
                # SWAP mode: add to swapped queue
                self.swapped_queue.append(req)
            else:
                # RECOMPUTE mode: add to waiting queue
                self._add_request_to_queue(req, is_retracted=True)
        
        return len(swapped_reqs)

    def _cfs_swap_out(self, req: Req) -> bool:
        """
        Swap out request's KV cache to CPU memory using HiCache.
        Returns True if successful, False otherwise.
        """
        try:
            if not hasattr(self.tree_cache, 'mem_pool_host') or self.tree_cache.mem_pool_host is None:
                return False
            
            mem_pool_host = self.tree_cache.mem_pool_host
            seqlen = req.seqlen
            
            # Allocate host memory
            if mem_pool_host.available_size() < seqlen:
                logger.warning(f"CFS SWAP: Not enough host memory for {seqlen} tokens")
                return False
            
            host_indices = mem_pool_host.alloc(seqlen)
            if host_indices is None or len(host_indices) < seqlen:
                return False
            
            # Get device indices for this request's KV cache
            device_indices = self.tree_cache.req_to_token_pool.req_to_token[
                req.req_pool_idx, :seqlen
            ]
            
            # Backup KV cache from GPU to CPU
            mem_pool_host.backup_from_device_all_layer(
                self.tree_cache.token_to_kv_pool,
                host_indices,
                device_indices,
                self.server_args.hicache_io_backend
            )
            
            # Store host indices for later swap-in
            self.swapped_host_indices[req.rid] = host_indices
            
            # Free GPU memory
            self.tree_cache.token_to_kv_pool_allocator.free(device_indices)
            
            return True
        except Exception as e:
            logger.warning(f"CFS SWAP out failed: {e}")
            return False

    def _cfs_swap_in(self, req: Req) -> bool:
        """
        Swap in request's KV cache from CPU memory back to GPU.
        Returns True if successful, False otherwise.
        """
        try:
            if req.rid not in self.swapped_host_indices:
                return False
            
            host_indices = self.swapped_host_indices[req.rid]
            mem_pool_host = self.tree_cache.mem_pool_host
            seqlen = len(host_indices)
            
            # Allocate GPU memory
            device_indices = self.tree_cache.token_to_kv_pool_allocator.alloc(seqlen)
            if device_indices is None:
                return False
            
            # Load KV cache from CPU to GPU (per layer)
            for layer_id in range(self.tree_cache.token_to_kv_pool.layer_num):
            ...
            # Update request's token indices
            self.tree_cache.req_to_token_pool.req_to_token[
                req.req_pool_idx, :seqlen
            ] = device_indices
            
            # Free host memory and remove from tracking
            mem_pool_host.free(host_indices)
            del self.swapped_host_indices[req.rid]
            
            return True
        except Exception as e:
            logger.warning(f"CFS SWAP in failed: {e}")
            return False
