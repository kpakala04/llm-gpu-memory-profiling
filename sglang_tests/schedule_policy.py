...
class CacheAgnosticPolicy(Enum):
    ...
    RANDOM = "random"
    CFS = "cfs"  # completely fair scheduler (shortest output first)
...
class SchedulePolicy:
    ...
    def _sort_by_cfs(waiting_queue: List[Req]) -> None:
        """CFS: Sorts by fewest output tokens generated (fairness).
        Requests with fewer generated tokens get higher priority."""
        waiting_queue.sort(key=lambda x: len(x.output_ids))
